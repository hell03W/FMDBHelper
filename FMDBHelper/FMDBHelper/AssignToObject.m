...
+ (id)customModel:(NSString *)model fromArr:(NSArray *)arr
{
    //获取类名
...
+ dictionaryWithModel:(id)model
{
    ...
    for (NSString *key in allKeys)
    {
        ...
        if (![model respondsToSelector:sel] || ![model performSelector:sel]) {
            [allValues addObject:@""];
        }else{
            [allValues addObject:[model performSelector:sel]];
        }
    }
...
{
    return [[self customModel:model fromArr:[NSArray arrayWithObject:dict]] firstObject];
}
